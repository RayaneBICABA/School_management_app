       ...
       <div class="max-w-[1600px] mx-auto flex flex-col gap-4">
          <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-4">
                 <!-- Role Filter Removed for Secretary -->
                ...
                <div v-else class="divide-y divide-slate-100 dark:divide-slate-800">
                    <div v-for="user in paginatedUsers" :key="user._id" class="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                        ...
                        <div class="col-span-2 flex justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                            <router-link v-if="user.role === 'ELEVE'" :to="`/secretaire/eleves/${user._id}`" class="p-2 text-slate-400 hover:text-primary transition-colors bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded shadow-sm hover:shadow" title="Voir la fiche">
...
const getAddUserLink = computed(() => {
    switch(currentType.value) {
        case 'parents': return '/secretaire/ajouter-utilisateur?role=PARENT'
        case 'eleves': return '/secretaire/ajouter-utilisateur?role=ELEVE'
        default: return '/secretaire/ajouter-utilisateur?role=PROFESSEUR'
    }
})
...
const filteredUsers = computed(() => {
  ...
  if (currentType.value === 'parents') {
      ...
      // Administration (For Secretary: Only Professors)
      filtered = filtered.filter(u => u.role === 'PROFESSEUR')
  }
...
// Use filteredUsers directly for display
const displayedUsers = computed(() => filteredUsers.value)

// For compatibility with template loop
