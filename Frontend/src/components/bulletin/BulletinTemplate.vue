<template>
  <div class="bulletin-container p-4 bg-gray-100 min-h-screen" :class="compactClasses">
    <div ref="bulletinRef" class="bulletin max-w-[900px] mx-auto bg-white p-8 shadow-lg text-[#333] font-serif leading-tight flex flex-col min-h-[297mm]">
      ...
      <!-- Grades Table -->
      <div class="flex-grow min-h-0 overflow-hidden mb-2 flex flex-col px-12 lg:px-20">
        <table class="w-full border-collapse border border-black text-[14px]">
          <thead>
            <tr class="bg-gray-200 text-center font-bold">
                <th class="border border-black p-4 text-left" style="width: 30%;">Matières</th>
                <th class="border border-black p-3 text-center" style="width: 8%;">Coef</th>
                <th class="border border-black p-3 text-center" style="width: 10%;">Moy</th>
                <th class="border border-black p-3 text-center" style="width: 10%;">Pondérées</th>
                <th class="border border-black p-3 text-center" colspan="3">Appréciations et signatures</th>
            </tr>
          </thead>
          <tbody>
            <template v-for="(category, catName) in groupedNotes" :key="catName">
              <tr class="bg-gray-300">
                <td :colspan="totalCols" class="border border-black p-4 font-bold text-center uppercase text-[13px]">{{ catName }}</td>
              </tr>
              <tr v-for="note in category" :key="note.matiere?._id" class="text-center">
                ...
                <td class="border border-black p-3">{{ (note.coeff || note.matiere?.coefficient || 0).toFixed(1) }}</td>
                
                <template v-if="note.isDispensed">
                  <td class="border border-black p-3 font-bold italic text-rose-600 text-center">D</td>
                  <td class="border border-black p-3 font-bold italic text-rose-600 text-center">D</td>
                </template>
                <template v-else>
                  <td class="border border-black p-3">{{ (note.moyenneMatiere || 0).toFixed(2) }}</td>
                  <td class="border border-black p-3 font-bold">{{ (note.notePonderee || 0).toFixed(2) }}</td>
                </template>
                <td class="border border-black p-3 w-20 italic text-[13px]" :class="note.isDispensed ? '' : getAppreciationColor(getSubjectAppreciation(note.moyenneMatiere || 0))">
                  {{ note.isDispensed ? '' : getSubjectAppreciation(note.moyenneMatiere || 0) }}
                </td>
                <td class="border border-black p-3 text-[12px]" style="width: 60px; white-space: nowrap;">{{ note.professeur ? (note.professeur.civilite === 'Mr' ? 'M ' : (note.professeur.civilite ? note.professeur.civilite + ' ' : '')) + (note.professeur.nom || '').toUpperCase() : '' }}</td>
                <td class="border border-black p-3 w-20"></td>
              </tr>
              <!-- Category Totals -->
              <tr class="bg-gray-100 font-bold">
                <td class="border border-black p-3 px-2 text-left uppercase text-[12px]">Total {{ catName }}</td>
                <td class="border border-black p-3 text-center">{{ getCategoryTotalCoeff(category) }}</td>
                <td class="border border-black p-3" colspan="1"></td>
                <td class="border border-black p-3 text-center">{{ getCategoryTotalPoints(category) }}</td>
                <td class="border border-black p-3" colspan="3"></td>
              </tr>
            </template>
            ...
            <tr class="bg-blue-50 font-bold text-center">
              ...
              <td class="border border-black p-3">{{ (bulletin.totalCoefficients || 0).toFixed(1) }}</td>
              <td class="border border-black p-3"></td>
              <td class="border border-black p-3 font-bold">{{ (bulletin.totalPoints || 0).toFixed(2) }}</td>
              <td class="border border-black p-3" colspan="3"></td>
            </tr>
          </tbody>
        </table>
